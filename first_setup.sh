#!/bin/bash
# ============================================================
# first_setup.sh - multi-agent-grid 初回セットアップスクリプト
...
echo ""
echo "  ╔══════════════════════════════════════════════════════════════╗"
echo "  ║  🌐 multi-agent-grid インストーラー                          ║"
echo "  ║     Initial Setup Script for Ubuntu / WSL / Mac              ║"
echo "  ╚══════════════════════════════════════════════════════════════╝"
echo ""
...
# multi-agent-grid 設定ファイル

# 言語設定
# ja: 日本語（サイバーパンク風日本語のみ、併記なし）
# en: 英語（サイバーパンク風 + 英訳併記）
...
# スキル設定
skill:
  # スキル保存先（スキル名に grid- プレフィックスを付けて保存）
...
# ============================================================
# STEP 8: Agent用タスク・レポートファイル初期化
# ============================================================
log_step "STEP 8: キューファイル初期化"

# Agent用タスクファイル作成
for i in {1..8}; do
    TASK_FILE="$SCRIPT_DIR/queue/tasks/a${i}.yaml"
    if [ ! -f "$TASK_FILE" ]; then
        cat > "$TASK_FILE" << EOF
# Agent ${i}専用タスクファイル
task:
...
log_info "Agentタスクファイル (1-8) を確認/作成しました"

# Agent用レポートファイル作成
for i in {1..8}; do
    REPORT_FILE="$SCRIPT_DIR/queue/reports/a${i}_report.yaml"
...
log_info "Agentレポートファイル (1-8) を確認/作成しました"
...
if [ -f "$SHELL_RC_FILE" ]; then
    ...
    if ! grep -q "alias csb=" "$SHELL_RC_FILE" 2>/dev/null; then
        # alias が存在しない → 新規追加
        echo "" >> "$SHELL_RC_FILE"
        echo "# multi-agent-grid aliases (added by first_setup.sh)" >> "$SHELL_RC_FILE"
        echo "$EXPECTED_CSB" >> "$SHELL_RC_FILE"
        log_info "alias csb を追加しました（Bossウィンドウの起動）"
        ALIAS_ADDED=true
    elif ! grep -qF "$EXPECTED_CSB" "$SHELL_RC_FILE" 2>/dev/null; then
        # alias は存在するがパスが異なる → 更新
        if sed_in_place "s|alias csb=.*|$EXPECTED_CSB|" "$SHELL_RC_FILE" 2>/dev/null; then
    ...
    if ! grep -q "alias csg=" "$SHELL_RC_FILE" 2>/dev/null; then
        if [ "$ALIAS_ADDED" = false ]; then
            echo "" >> "$SHELL_RC_FILE"
            echo "# multi-agent-grid aliases (added by first_setup.sh)" >> "$SHELL_RC_FILE"
        fi
        echo "$EXPECTED_CSG" >> "$SHELL_RC_FILE"
        log_info "alias csg を追加しました（Operator・Agentウィンドウの起動）"
        ALIAS_ADDED=true
    elif ! grep -qF "$EXPECTED_CSG" "$SHELL_RC_FILE" 2>/dev/null; then
        if sed_in_place "s|alias csg=.*|$EXPECTED_CSG|" "$SHELL_RC_FILE" 2>/dev/null; then
...
if command -v claude &> /dev/null; then
    ...
    if claude mcp list 2>/dev/null | grep -q "memory"; then
        ...
        if claude mcp add memory \
            -e MEMORY_FILE_PATH="$SCRIPT_DIR/memory/grid_memory.jsonl" \
...
echo ""
echo "  オプション:"
echo "     ./deploy.sh -s            # セットアップのみ（Claude手動起動）"
echo "     ./deploy.sh -t            # Windows Terminalタブ展開"
echo "     ./deploy.sh -shell bash   # bash用プロンプトで起動"
echo "     ./deploy.sh -shell zsh    # zsh用プロンプトで起動"
echo ""
